...
//
// Everything here runs on the END USER's own access token, so it needs no
// machine-to-machine credentials. This app has no Management API app
// registered, so — unlike Cloud's equivalent — `hasPassword` and linked
// social `identities` come straight from `/api/my-account` (gated by the
// Logto Account Center's `fields` config and the token's `identities`
// scope) rather than a Management API fallback. Half-wiring an M2M client
// would fail at runtime rather than at build, so it's left out entirely.
//
